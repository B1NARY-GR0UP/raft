// Code generated by thriftgo (0.3.15). DO NOT EDIT.

package raft

import (
	"bytes"
	"database/sql"
	"database/sql/driver"
	"fmt"
	"github.com/apache/thrift/lib/go/thrift"
)

type EntryType int64

const (
	EntryType_Normal EntryType = 0
	EntryType_Config EntryType = 1
)

func (p EntryType) String() string {
	switch p {
	case EntryType_Normal:
		return "Normal"
	case EntryType_Config:
		return "Config"
	}
	return "<UNSET>"
}

func EntryTypeFromString(s string) (EntryType, error) {
	switch s {
	case "Normal":
		return EntryType_Normal, nil
	case "Config":
		return EntryType_Config, nil
	}
	return EntryType(0), fmt.Errorf("not a valid EntryType string")
}

func EntryTypePtr(v EntryType) *EntryType { return &v }
func (p *EntryType) Scan(value interface{}) (err error) {
	var result sql.NullInt64
	err = result.Scan(value)
	*p = EntryType(result.Int64)
	return
}

func (p *EntryType) Value() (driver.Value, error) {
	if p == nil {
		return nil, nil
	}
	return int64(*p), nil
}

type MessageType int64

const (
	MessageType_Propose             MessageType = 0
	MessageType_Campaign            MessageType = 1
	MessageType_Beat                MessageType = 2
	MessageType_StorageAppend       MessageType = 3
	MessageType_StorageApply        MessageType = 4
	MessageType_AppendEntries       MessageType = 5
	MessageType_AppendEntriesResp   MessageType = 6
	MessageType_RequestVote         MessageType = 7
	MessageType_RequestVoteResp     MessageType = 8
	MessageType_InstallSnapshot     MessageType = 9
	MessageType_InstallSnapshotResp MessageType = 10
)

func (p MessageType) String() string {
	switch p {
	case MessageType_Propose:
		return "Propose"
	case MessageType_Campaign:
		return "Campaign"
	case MessageType_Beat:
		return "Beat"
	case MessageType_StorageAppend:
		return "StorageAppend"
	case MessageType_StorageApply:
		return "StorageApply"
	case MessageType_AppendEntries:
		return "AppendEntries"
	case MessageType_AppendEntriesResp:
		return "AppendEntriesResp"
	case MessageType_RequestVote:
		return "RequestVote"
	case MessageType_RequestVoteResp:
		return "RequestVoteResp"
	case MessageType_InstallSnapshot:
		return "InstallSnapshot"
	case MessageType_InstallSnapshotResp:
		return "InstallSnapshotResp"
	}
	return "<UNSET>"
}

func MessageTypeFromString(s string) (MessageType, error) {
	switch s {
	case "Propose":
		return MessageType_Propose, nil
	case "Campaign":
		return MessageType_Campaign, nil
	case "Beat":
		return MessageType_Beat, nil
	case "StorageAppend":
		return MessageType_StorageAppend, nil
	case "StorageApply":
		return MessageType_StorageApply, nil
	case "AppendEntries":
		return MessageType_AppendEntries, nil
	case "AppendEntriesResp":
		return MessageType_AppendEntriesResp, nil
	case "RequestVote":
		return MessageType_RequestVote, nil
	case "RequestVoteResp":
		return MessageType_RequestVoteResp, nil
	case "InstallSnapshot":
		return MessageType_InstallSnapshot, nil
	case "InstallSnapshotResp":
		return MessageType_InstallSnapshotResp, nil
	}
	return MessageType(0), fmt.Errorf("not a valid MessageType string")
}

func MessageTypePtr(v MessageType) *MessageType { return &v }
func (p *MessageType) Scan(value interface{}) (err error) {
	var result sql.NullInt64
	err = result.Scan(value)
	*p = MessageType(result.Int64)
	return
}

func (p *MessageType) Value() (driver.Value, error) {
	if p == nil {
		return nil, nil
	}
	return int64(*p), nil
}

type ConfChangeType int64

const (
	ConfChangeType_AddNode    ConfChangeType = 0
	ConfChangeType_RemoveNode ConfChangeType = 1
)

func (p ConfChangeType) String() string {
	switch p {
	case ConfChangeType_AddNode:
		return "AddNode"
	case ConfChangeType_RemoveNode:
		return "RemoveNode"
	}
	return "<UNSET>"
}

func ConfChangeTypeFromString(s string) (ConfChangeType, error) {
	switch s {
	case "AddNode":
		return ConfChangeType_AddNode, nil
	case "RemoveNode":
		return ConfChangeType_RemoveNode, nil
	}
	return ConfChangeType(0), fmt.Errorf("not a valid ConfChangeType string")
}

func ConfChangeTypePtr(v ConfChangeType) *ConfChangeType { return &v }
func (p *ConfChangeType) Scan(value interface{}) (err error) {
	var result sql.NullInt64
	err = result.Scan(value)
	*p = ConfChangeType(result.Int64)
	return
}

func (p *ConfChangeType) Value() (driver.Value, error) {
	if p == nil {
		return nil, nil
	}
	return int64(*p), nil
}

type ConfChangeTransition int64

const (
	ConfChangeTransition_Auto          ConfChangeTransition = 0
	ConfChangeTransition_JointImplicit ConfChangeTransition = 1
	ConfChangeTransition_JointExplicit ConfChangeTransition = 2
)

func (p ConfChangeTransition) String() string {
	switch p {
	case ConfChangeTransition_Auto:
		return "Auto"
	case ConfChangeTransition_JointImplicit:
		return "JointImplicit"
	case ConfChangeTransition_JointExplicit:
		return "JointExplicit"
	}
	return "<UNSET>"
}

func ConfChangeTransitionFromString(s string) (ConfChangeTransition, error) {
	switch s {
	case "Auto":
		return ConfChangeTransition_Auto, nil
	case "JointImplicit":
		return ConfChangeTransition_JointImplicit, nil
	case "JointExplicit":
		return ConfChangeTransition_JointExplicit, nil
	}
	return ConfChangeTransition(0), fmt.Errorf("not a valid ConfChangeTransition string")
}

func ConfChangeTransitionPtr(v ConfChangeTransition) *ConfChangeTransition { return &v }
func (p *ConfChangeTransition) Scan(value interface{}) (err error) {
	var result sql.NullInt64
	err = result.Scan(value)
	*p = ConfChangeTransition(result.Int64)
	return
}

func (p *ConfChangeTransition) Value() (driver.Value, error) {
	if p == nil {
		return nil, nil
	}
	return int64(*p), nil
}

type Entry struct {
	Type  EntryType `thrift:"type,1" frugal:"1,default,EntryType" json:"type"`
	Term  int64     `thrift:"term,2" frugal:"2,default,i64" json:"term"`
	Index int64     `thrift:"index,3" frugal:"3,default,i64" json:"index"`
	Data  []byte    `thrift:"data,4" frugal:"4,default,binary" json:"data"`
}

func NewEntry() *Entry {
	return &Entry{}
}

func (p *Entry) InitDefault() {
}

func (p *Entry) GetType() (v EntryType) {
	return p.Type
}

func (p *Entry) GetTerm() (v int64) {
	return p.Term
}

func (p *Entry) GetIndex() (v int64) {
	return p.Index
}

func (p *Entry) GetData() (v []byte) {
	return p.Data
}
func (p *Entry) SetType(val EntryType) {
	p.Type = val
}
func (p *Entry) SetTerm(val int64) {
	p.Term = val
}
func (p *Entry) SetIndex(val int64) {
	p.Index = val
}
func (p *Entry) SetData(val []byte) {
	p.Data = val
}

var fieldIDToName_Entry = map[int16]string{
	1: "type",
	2: "term",
	3: "index",
	4: "data",
}

func (p *Entry) Read(iprot thrift.TProtocol) (err error) {

	var fieldTypeId thrift.TType
	var fieldId int16

	if _, err = iprot.ReadStructBegin(); err != nil {
		goto ReadStructBeginError
	}

	for {
		_, fieldTypeId, fieldId, err = iprot.ReadFieldBegin()
		if err != nil {
			goto ReadFieldBeginError
		}
		if fieldTypeId == thrift.STOP {
			break
		}

		switch fieldId {
		case 1:
			if fieldTypeId == thrift.I32 {
				if err = p.ReadField1(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 2:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField2(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 3:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField3(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 4:
			if fieldTypeId == thrift.STRING {
				if err = p.ReadField4(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		default:
			if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		}
		if err = iprot.ReadFieldEnd(); err != nil {
			goto ReadFieldEndError
		}
	}
	if err = iprot.ReadStructEnd(); err != nil {
		goto ReadStructEndError
	}

	return nil
ReadStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read struct begin error: ", p), err)
ReadFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d begin error: ", p, fieldId), err)
ReadFieldError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d '%s' error: ", p, fieldId, fieldIDToName_Entry[fieldId]), err)
SkipFieldError:
	return thrift.PrependError(fmt.Sprintf("%T field %d skip type %d error: ", p, fieldId, fieldTypeId), err)

ReadFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T read field end error", p), err)
ReadStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T read struct end error: ", p), err)
}

func (p *Entry) ReadField1(iprot thrift.TProtocol) error {

	var _field EntryType
	if v, err := iprot.ReadI32(); err != nil {
		return err
	} else {
		_field = EntryType(v)
	}
	p.Type = _field
	return nil
}
func (p *Entry) ReadField2(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.Term = _field
	return nil
}
func (p *Entry) ReadField3(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.Index = _field
	return nil
}
func (p *Entry) ReadField4(iprot thrift.TProtocol) error {

	var _field []byte
	if v, err := iprot.ReadBinary(); err != nil {
		return err
	} else {
		_field = []byte(v)
	}
	p.Data = _field
	return nil
}

func (p *Entry) Write(oprot thrift.TProtocol) (err error) {
	var fieldId int16
	if err = oprot.WriteStructBegin("Entry"); err != nil {
		goto WriteStructBeginError
	}
	if p != nil {
		if err = p.writeField1(oprot); err != nil {
			fieldId = 1
			goto WriteFieldError
		}
		if err = p.writeField2(oprot); err != nil {
			fieldId = 2
			goto WriteFieldError
		}
		if err = p.writeField3(oprot); err != nil {
			fieldId = 3
			goto WriteFieldError
		}
		if err = p.writeField4(oprot); err != nil {
			fieldId = 4
			goto WriteFieldError
		}
	}
	if err = oprot.WriteFieldStop(); err != nil {
		goto WriteFieldStopError
	}
	if err = oprot.WriteStructEnd(); err != nil {
		goto WriteStructEndError
	}
	return nil
WriteStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write struct begin error: ", p), err)
WriteFieldError:
	return thrift.PrependError(fmt.Sprintf("%T write field %d error: ", p, fieldId), err)
WriteFieldStopError:
	return thrift.PrependError(fmt.Sprintf("%T write field stop error: ", p), err)
WriteStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T write struct end error: ", p), err)
}

func (p *Entry) writeField1(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("type", thrift.I32, 1); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI32(int32(p.Type)); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 end error: ", p), err)
}

func (p *Entry) writeField2(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("term", thrift.I64, 2); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.Term); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 end error: ", p), err)
}

func (p *Entry) writeField3(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("index", thrift.I64, 3); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.Index); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 3 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 3 end error: ", p), err)
}

func (p *Entry) writeField4(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("data", thrift.STRING, 4); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteBinary([]byte(p.Data)); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 4 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 4 end error: ", p), err)
}

func (p *Entry) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("Entry(%+v)", *p)

}

func (p *Entry) DeepEqual(ano *Entry) bool {
	if p == ano {
		return true
	} else if p == nil || ano == nil {
		return false
	}
	if !p.Field1DeepEqual(ano.Type) {
		return false
	}
	if !p.Field2DeepEqual(ano.Term) {
		return false
	}
	if !p.Field3DeepEqual(ano.Index) {
		return false
	}
	if !p.Field4DeepEqual(ano.Data) {
		return false
	}
	return true
}

func (p *Entry) Field1DeepEqual(src EntryType) bool {

	if p.Type != src {
		return false
	}
	return true
}
func (p *Entry) Field2DeepEqual(src int64) bool {

	if p.Term != src {
		return false
	}
	return true
}
func (p *Entry) Field3DeepEqual(src int64) bool {

	if p.Index != src {
		return false
	}
	return true
}
func (p *Entry) Field4DeepEqual(src []byte) bool {

	if bytes.Compare(p.Data, src) != 0 {
		return false
	}
	return true
}

type PersistentState struct {
	CurrentTerm int64 `thrift:"current_term,1" frugal:"1,default,i64" json:"current_term"`
	VotedFor    int64 `thrift:"voted_for,2" frugal:"2,default,i64" json:"voted_for"`
}

func NewPersistentState() *PersistentState {
	return &PersistentState{}
}

func (p *PersistentState) InitDefault() {
}

func (p *PersistentState) GetCurrentTerm() (v int64) {
	return p.CurrentTerm
}

func (p *PersistentState) GetVotedFor() (v int64) {
	return p.VotedFor
}
func (p *PersistentState) SetCurrentTerm(val int64) {
	p.CurrentTerm = val
}
func (p *PersistentState) SetVotedFor(val int64) {
	p.VotedFor = val
}

var fieldIDToName_PersistentState = map[int16]string{
	1: "current_term",
	2: "voted_for",
}

func (p *PersistentState) Read(iprot thrift.TProtocol) (err error) {

	var fieldTypeId thrift.TType
	var fieldId int16

	if _, err = iprot.ReadStructBegin(); err != nil {
		goto ReadStructBeginError
	}

	for {
		_, fieldTypeId, fieldId, err = iprot.ReadFieldBegin()
		if err != nil {
			goto ReadFieldBeginError
		}
		if fieldTypeId == thrift.STOP {
			break
		}

		switch fieldId {
		case 1:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField1(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 2:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField2(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		default:
			if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		}
		if err = iprot.ReadFieldEnd(); err != nil {
			goto ReadFieldEndError
		}
	}
	if err = iprot.ReadStructEnd(); err != nil {
		goto ReadStructEndError
	}

	return nil
ReadStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read struct begin error: ", p), err)
ReadFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d begin error: ", p, fieldId), err)
ReadFieldError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d '%s' error: ", p, fieldId, fieldIDToName_PersistentState[fieldId]), err)
SkipFieldError:
	return thrift.PrependError(fmt.Sprintf("%T field %d skip type %d error: ", p, fieldId, fieldTypeId), err)

ReadFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T read field end error", p), err)
ReadStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T read struct end error: ", p), err)
}

func (p *PersistentState) ReadField1(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.CurrentTerm = _field
	return nil
}
func (p *PersistentState) ReadField2(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.VotedFor = _field
	return nil
}

func (p *PersistentState) Write(oprot thrift.TProtocol) (err error) {
	var fieldId int16
	if err = oprot.WriteStructBegin("PersistentState"); err != nil {
		goto WriteStructBeginError
	}
	if p != nil {
		if err = p.writeField1(oprot); err != nil {
			fieldId = 1
			goto WriteFieldError
		}
		if err = p.writeField2(oprot); err != nil {
			fieldId = 2
			goto WriteFieldError
		}
	}
	if err = oprot.WriteFieldStop(); err != nil {
		goto WriteFieldStopError
	}
	if err = oprot.WriteStructEnd(); err != nil {
		goto WriteStructEndError
	}
	return nil
WriteStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write struct begin error: ", p), err)
WriteFieldError:
	return thrift.PrependError(fmt.Sprintf("%T write field %d error: ", p, fieldId), err)
WriteFieldStopError:
	return thrift.PrependError(fmt.Sprintf("%T write field stop error: ", p), err)
WriteStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T write struct end error: ", p), err)
}

func (p *PersistentState) writeField1(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("current_term", thrift.I64, 1); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.CurrentTerm); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 end error: ", p), err)
}

func (p *PersistentState) writeField2(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("voted_for", thrift.I64, 2); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.VotedFor); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 end error: ", p), err)
}

func (p *PersistentState) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("PersistentState(%+v)", *p)

}

func (p *PersistentState) DeepEqual(ano *PersistentState) bool {
	if p == ano {
		return true
	} else if p == nil || ano == nil {
		return false
	}
	if !p.Field1DeepEqual(ano.CurrentTerm) {
		return false
	}
	if !p.Field2DeepEqual(ano.VotedFor) {
		return false
	}
	return true
}

func (p *PersistentState) Field1DeepEqual(src int64) bool {

	if p.CurrentTerm != src {
		return false
	}
	return true
}
func (p *PersistentState) Field2DeepEqual(src int64) bool {

	if p.VotedFor != src {
		return false
	}
	return true
}

type ConfState struct {
	Voters         []int64 `thrift:"voters,1" frugal:"1,default,list<i64>" json:"voters"`
	VotersOutgoing []int64 `thrift:"voters_outgoing,2" frugal:"2,default,list<i64>" json:"voters_outgoing"`
	AutoLeave      bool    `thrift:"auto_leave,3" frugal:"3,default,bool" json:"auto_leave"`
}

func NewConfState() *ConfState {
	return &ConfState{}
}

func (p *ConfState) InitDefault() {
}

func (p *ConfState) GetVoters() (v []int64) {
	return p.Voters
}

func (p *ConfState) GetVotersOutgoing() (v []int64) {
	return p.VotersOutgoing
}

func (p *ConfState) GetAutoLeave() (v bool) {
	return p.AutoLeave
}
func (p *ConfState) SetVoters(val []int64) {
	p.Voters = val
}
func (p *ConfState) SetVotersOutgoing(val []int64) {
	p.VotersOutgoing = val
}
func (p *ConfState) SetAutoLeave(val bool) {
	p.AutoLeave = val
}

var fieldIDToName_ConfState = map[int16]string{
	1: "voters",
	2: "voters_outgoing",
	3: "auto_leave",
}

func (p *ConfState) Read(iprot thrift.TProtocol) (err error) {

	var fieldTypeId thrift.TType
	var fieldId int16

	if _, err = iprot.ReadStructBegin(); err != nil {
		goto ReadStructBeginError
	}

	for {
		_, fieldTypeId, fieldId, err = iprot.ReadFieldBegin()
		if err != nil {
			goto ReadFieldBeginError
		}
		if fieldTypeId == thrift.STOP {
			break
		}

		switch fieldId {
		case 1:
			if fieldTypeId == thrift.LIST {
				if err = p.ReadField1(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 2:
			if fieldTypeId == thrift.LIST {
				if err = p.ReadField2(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 3:
			if fieldTypeId == thrift.BOOL {
				if err = p.ReadField3(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		default:
			if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		}
		if err = iprot.ReadFieldEnd(); err != nil {
			goto ReadFieldEndError
		}
	}
	if err = iprot.ReadStructEnd(); err != nil {
		goto ReadStructEndError
	}

	return nil
ReadStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read struct begin error: ", p), err)
ReadFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d begin error: ", p, fieldId), err)
ReadFieldError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d '%s' error: ", p, fieldId, fieldIDToName_ConfState[fieldId]), err)
SkipFieldError:
	return thrift.PrependError(fmt.Sprintf("%T field %d skip type %d error: ", p, fieldId, fieldTypeId), err)

ReadFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T read field end error", p), err)
ReadStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T read struct end error: ", p), err)
}

func (p *ConfState) ReadField1(iprot thrift.TProtocol) error {
	_, size, err := iprot.ReadListBegin()
	if err != nil {
		return err
	}
	_field := make([]int64, 0, size)
	for i := 0; i < size; i++ {

		var _elem int64
		if v, err := iprot.ReadI64(); err != nil {
			return err
		} else {
			_elem = v
		}

		_field = append(_field, _elem)
	}
	if err := iprot.ReadListEnd(); err != nil {
		return err
	}
	p.Voters = _field
	return nil
}
func (p *ConfState) ReadField2(iprot thrift.TProtocol) error {
	_, size, err := iprot.ReadListBegin()
	if err != nil {
		return err
	}
	_field := make([]int64, 0, size)
	for i := 0; i < size; i++ {

		var _elem int64
		if v, err := iprot.ReadI64(); err != nil {
			return err
		} else {
			_elem = v
		}

		_field = append(_field, _elem)
	}
	if err := iprot.ReadListEnd(); err != nil {
		return err
	}
	p.VotersOutgoing = _field
	return nil
}
func (p *ConfState) ReadField3(iprot thrift.TProtocol) error {

	var _field bool
	if v, err := iprot.ReadBool(); err != nil {
		return err
	} else {
		_field = v
	}
	p.AutoLeave = _field
	return nil
}

func (p *ConfState) Write(oprot thrift.TProtocol) (err error) {
	var fieldId int16
	if err = oprot.WriteStructBegin("ConfState"); err != nil {
		goto WriteStructBeginError
	}
	if p != nil {
		if err = p.writeField1(oprot); err != nil {
			fieldId = 1
			goto WriteFieldError
		}
		if err = p.writeField2(oprot); err != nil {
			fieldId = 2
			goto WriteFieldError
		}
		if err = p.writeField3(oprot); err != nil {
			fieldId = 3
			goto WriteFieldError
		}
	}
	if err = oprot.WriteFieldStop(); err != nil {
		goto WriteFieldStopError
	}
	if err = oprot.WriteStructEnd(); err != nil {
		goto WriteStructEndError
	}
	return nil
WriteStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write struct begin error: ", p), err)
WriteFieldError:
	return thrift.PrependError(fmt.Sprintf("%T write field %d error: ", p, fieldId), err)
WriteFieldStopError:
	return thrift.PrependError(fmt.Sprintf("%T write field stop error: ", p), err)
WriteStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T write struct end error: ", p), err)
}

func (p *ConfState) writeField1(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("voters", thrift.LIST, 1); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteListBegin(thrift.I64, len(p.Voters)); err != nil {
		return err
	}
	for _, v := range p.Voters {
		if err := oprot.WriteI64(v); err != nil {
			return err
		}
	}
	if err := oprot.WriteListEnd(); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 end error: ", p), err)
}

func (p *ConfState) writeField2(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("voters_outgoing", thrift.LIST, 2); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteListBegin(thrift.I64, len(p.VotersOutgoing)); err != nil {
		return err
	}
	for _, v := range p.VotersOutgoing {
		if err := oprot.WriteI64(v); err != nil {
			return err
		}
	}
	if err := oprot.WriteListEnd(); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 end error: ", p), err)
}

func (p *ConfState) writeField3(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("auto_leave", thrift.BOOL, 3); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteBool(p.AutoLeave); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 3 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 3 end error: ", p), err)
}

func (p *ConfState) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("ConfState(%+v)", *p)

}

func (p *ConfState) DeepEqual(ano *ConfState) bool {
	if p == ano {
		return true
	} else if p == nil || ano == nil {
		return false
	}
	if !p.Field1DeepEqual(ano.Voters) {
		return false
	}
	if !p.Field2DeepEqual(ano.VotersOutgoing) {
		return false
	}
	if !p.Field3DeepEqual(ano.AutoLeave) {
		return false
	}
	return true
}

func (p *ConfState) Field1DeepEqual(src []int64) bool {

	if len(p.Voters) != len(src) {
		return false
	}
	for i, v := range p.Voters {
		_src := src[i]
		if v != _src {
			return false
		}
	}
	return true
}
func (p *ConfState) Field2DeepEqual(src []int64) bool {

	if len(p.VotersOutgoing) != len(src) {
		return false
	}
	for i, v := range p.VotersOutgoing {
		_src := src[i]
		if v != _src {
			return false
		}
	}
	return true
}
func (p *ConfState) Field3DeepEqual(src bool) bool {

	if p.AutoLeave != src {
		return false
	}
	return true
}

type SnapshotMetadata struct {
	ConfState *ConfState `thrift:"conf_state,1" frugal:"1,default,ConfState" json:"conf_state"`
	LastIndex int64      `thrift:"last_index,2" frugal:"2,default,i64" json:"last_index"`
	LastTerm  int64      `thrift:"last_term,3" frugal:"3,default,i64" json:"last_term"`
}

func NewSnapshotMetadata() *SnapshotMetadata {
	return &SnapshotMetadata{}
}

func (p *SnapshotMetadata) InitDefault() {
}

var SnapshotMetadata_ConfState_DEFAULT *ConfState

func (p *SnapshotMetadata) GetConfState() (v *ConfState) {
	if !p.IsSetConfState() {
		return SnapshotMetadata_ConfState_DEFAULT
	}
	return p.ConfState
}

func (p *SnapshotMetadata) GetLastIndex() (v int64) {
	return p.LastIndex
}

func (p *SnapshotMetadata) GetLastTerm() (v int64) {
	return p.LastTerm
}
func (p *SnapshotMetadata) SetConfState(val *ConfState) {
	p.ConfState = val
}
func (p *SnapshotMetadata) SetLastIndex(val int64) {
	p.LastIndex = val
}
func (p *SnapshotMetadata) SetLastTerm(val int64) {
	p.LastTerm = val
}

var fieldIDToName_SnapshotMetadata = map[int16]string{
	1: "conf_state",
	2: "last_index",
	3: "last_term",
}

func (p *SnapshotMetadata) IsSetConfState() bool {
	return p.ConfState != nil
}

func (p *SnapshotMetadata) Read(iprot thrift.TProtocol) (err error) {

	var fieldTypeId thrift.TType
	var fieldId int16

	if _, err = iprot.ReadStructBegin(); err != nil {
		goto ReadStructBeginError
	}

	for {
		_, fieldTypeId, fieldId, err = iprot.ReadFieldBegin()
		if err != nil {
			goto ReadFieldBeginError
		}
		if fieldTypeId == thrift.STOP {
			break
		}

		switch fieldId {
		case 1:
			if fieldTypeId == thrift.STRUCT {
				if err = p.ReadField1(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 2:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField2(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 3:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField3(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		default:
			if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		}
		if err = iprot.ReadFieldEnd(); err != nil {
			goto ReadFieldEndError
		}
	}
	if err = iprot.ReadStructEnd(); err != nil {
		goto ReadStructEndError
	}

	return nil
ReadStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read struct begin error: ", p), err)
ReadFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d begin error: ", p, fieldId), err)
ReadFieldError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d '%s' error: ", p, fieldId, fieldIDToName_SnapshotMetadata[fieldId]), err)
SkipFieldError:
	return thrift.PrependError(fmt.Sprintf("%T field %d skip type %d error: ", p, fieldId, fieldTypeId), err)

ReadFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T read field end error", p), err)
ReadStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T read struct end error: ", p), err)
}

func (p *SnapshotMetadata) ReadField1(iprot thrift.TProtocol) error {
	_field := NewConfState()
	if err := _field.Read(iprot); err != nil {
		return err
	}
	p.ConfState = _field
	return nil
}
func (p *SnapshotMetadata) ReadField2(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.LastIndex = _field
	return nil
}
func (p *SnapshotMetadata) ReadField3(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.LastTerm = _field
	return nil
}

func (p *SnapshotMetadata) Write(oprot thrift.TProtocol) (err error) {
	var fieldId int16
	if err = oprot.WriteStructBegin("SnapshotMetadata"); err != nil {
		goto WriteStructBeginError
	}
	if p != nil {
		if err = p.writeField1(oprot); err != nil {
			fieldId = 1
			goto WriteFieldError
		}
		if err = p.writeField2(oprot); err != nil {
			fieldId = 2
			goto WriteFieldError
		}
		if err = p.writeField3(oprot); err != nil {
			fieldId = 3
			goto WriteFieldError
		}
	}
	if err = oprot.WriteFieldStop(); err != nil {
		goto WriteFieldStopError
	}
	if err = oprot.WriteStructEnd(); err != nil {
		goto WriteStructEndError
	}
	return nil
WriteStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write struct begin error: ", p), err)
WriteFieldError:
	return thrift.PrependError(fmt.Sprintf("%T write field %d error: ", p, fieldId), err)
WriteFieldStopError:
	return thrift.PrependError(fmt.Sprintf("%T write field stop error: ", p), err)
WriteStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T write struct end error: ", p), err)
}

func (p *SnapshotMetadata) writeField1(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("conf_state", thrift.STRUCT, 1); err != nil {
		goto WriteFieldBeginError
	}
	if err := p.ConfState.Write(oprot); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 end error: ", p), err)
}

func (p *SnapshotMetadata) writeField2(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("last_index", thrift.I64, 2); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.LastIndex); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 end error: ", p), err)
}

func (p *SnapshotMetadata) writeField3(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("last_term", thrift.I64, 3); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.LastTerm); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 3 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 3 end error: ", p), err)
}

func (p *SnapshotMetadata) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("SnapshotMetadata(%+v)", *p)

}

func (p *SnapshotMetadata) DeepEqual(ano *SnapshotMetadata) bool {
	if p == ano {
		return true
	} else if p == nil || ano == nil {
		return false
	}
	if !p.Field1DeepEqual(ano.ConfState) {
		return false
	}
	if !p.Field2DeepEqual(ano.LastIndex) {
		return false
	}
	if !p.Field3DeepEqual(ano.LastTerm) {
		return false
	}
	return true
}

func (p *SnapshotMetadata) Field1DeepEqual(src *ConfState) bool {

	if !p.ConfState.DeepEqual(src) {
		return false
	}
	return true
}
func (p *SnapshotMetadata) Field2DeepEqual(src int64) bool {

	if p.LastIndex != src {
		return false
	}
	return true
}
func (p *SnapshotMetadata) Field3DeepEqual(src int64) bool {

	if p.LastTerm != src {
		return false
	}
	return true
}

type Snapshot struct {
	Metadata *SnapshotMetadata `thrift:"metadata,1" frugal:"1,default,SnapshotMetadata" json:"metadata"`
	Data     []byte            `thrift:"data,2" frugal:"2,default,binary" json:"data"`
}

func NewSnapshot() *Snapshot {
	return &Snapshot{}
}

func (p *Snapshot) InitDefault() {
}

var Snapshot_Metadata_DEFAULT *SnapshotMetadata

func (p *Snapshot) GetMetadata() (v *SnapshotMetadata) {
	if !p.IsSetMetadata() {
		return Snapshot_Metadata_DEFAULT
	}
	return p.Metadata
}

func (p *Snapshot) GetData() (v []byte) {
	return p.Data
}
func (p *Snapshot) SetMetadata(val *SnapshotMetadata) {
	p.Metadata = val
}
func (p *Snapshot) SetData(val []byte) {
	p.Data = val
}

var fieldIDToName_Snapshot = map[int16]string{
	1: "metadata",
	2: "data",
}

func (p *Snapshot) IsSetMetadata() bool {
	return p.Metadata != nil
}

func (p *Snapshot) Read(iprot thrift.TProtocol) (err error) {

	var fieldTypeId thrift.TType
	var fieldId int16

	if _, err = iprot.ReadStructBegin(); err != nil {
		goto ReadStructBeginError
	}

	for {
		_, fieldTypeId, fieldId, err = iprot.ReadFieldBegin()
		if err != nil {
			goto ReadFieldBeginError
		}
		if fieldTypeId == thrift.STOP {
			break
		}

		switch fieldId {
		case 1:
			if fieldTypeId == thrift.STRUCT {
				if err = p.ReadField1(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 2:
			if fieldTypeId == thrift.STRING {
				if err = p.ReadField2(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		default:
			if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		}
		if err = iprot.ReadFieldEnd(); err != nil {
			goto ReadFieldEndError
		}
	}
	if err = iprot.ReadStructEnd(); err != nil {
		goto ReadStructEndError
	}

	return nil
ReadStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read struct begin error: ", p), err)
ReadFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d begin error: ", p, fieldId), err)
ReadFieldError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d '%s' error: ", p, fieldId, fieldIDToName_Snapshot[fieldId]), err)
SkipFieldError:
	return thrift.PrependError(fmt.Sprintf("%T field %d skip type %d error: ", p, fieldId, fieldTypeId), err)

ReadFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T read field end error", p), err)
ReadStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T read struct end error: ", p), err)
}

func (p *Snapshot) ReadField1(iprot thrift.TProtocol) error {
	_field := NewSnapshotMetadata()
	if err := _field.Read(iprot); err != nil {
		return err
	}
	p.Metadata = _field
	return nil
}
func (p *Snapshot) ReadField2(iprot thrift.TProtocol) error {

	var _field []byte
	if v, err := iprot.ReadBinary(); err != nil {
		return err
	} else {
		_field = []byte(v)
	}
	p.Data = _field
	return nil
}

func (p *Snapshot) Write(oprot thrift.TProtocol) (err error) {
	var fieldId int16
	if err = oprot.WriteStructBegin("Snapshot"); err != nil {
		goto WriteStructBeginError
	}
	if p != nil {
		if err = p.writeField1(oprot); err != nil {
			fieldId = 1
			goto WriteFieldError
		}
		if err = p.writeField2(oprot); err != nil {
			fieldId = 2
			goto WriteFieldError
		}
	}
	if err = oprot.WriteFieldStop(); err != nil {
		goto WriteFieldStopError
	}
	if err = oprot.WriteStructEnd(); err != nil {
		goto WriteStructEndError
	}
	return nil
WriteStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write struct begin error: ", p), err)
WriteFieldError:
	return thrift.PrependError(fmt.Sprintf("%T write field %d error: ", p, fieldId), err)
WriteFieldStopError:
	return thrift.PrependError(fmt.Sprintf("%T write field stop error: ", p), err)
WriteStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T write struct end error: ", p), err)
}

func (p *Snapshot) writeField1(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("metadata", thrift.STRUCT, 1); err != nil {
		goto WriteFieldBeginError
	}
	if err := p.Metadata.Write(oprot); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 end error: ", p), err)
}

func (p *Snapshot) writeField2(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("data", thrift.STRING, 2); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteBinary([]byte(p.Data)); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 end error: ", p), err)
}

func (p *Snapshot) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("Snapshot(%+v)", *p)

}

func (p *Snapshot) DeepEqual(ano *Snapshot) bool {
	if p == ano {
		return true
	} else if p == nil || ano == nil {
		return false
	}
	if !p.Field1DeepEqual(ano.Metadata) {
		return false
	}
	if !p.Field2DeepEqual(ano.Data) {
		return false
	}
	return true
}

func (p *Snapshot) Field1DeepEqual(src *SnapshotMetadata) bool {

	if !p.Metadata.DeepEqual(src) {
		return false
	}
	return true
}
func (p *Snapshot) Field2DeepEqual(src []byte) bool {

	if bytes.Compare(p.Data, src) != 0 {
		return false
	}
	return true
}

type Message struct {
	Type         MessageType `thrift:"type,1" frugal:"1,default,MessageType" json:"type"`
	Src          int64       `thrift:"src,2" frugal:"2,default,i64" json:"src"`
	Dst          int64       `thrift:"dst,3" frugal:"3,default,i64" json:"dst"`
	Term         int64       `thrift:"term,4" frugal:"4,default,i64" json:"term"`
	LogIndex     int64       `thrift:"log_index,5" frugal:"5,default,i64" json:"log_index"`
	LogTerm      int64       `thrift:"log_term,6" frugal:"6,default,i64" json:"log_term"`
	Entries      []*Entry    `thrift:"entries,7" frugal:"7,default,list<Entry>" json:"entries"`
	LeaderCommit int64       `thrift:"leader_commit,8" frugal:"8,default,i64" json:"leader_commit"`
	Snapshot     *Snapshot   `thrift:"snapshot,9" frugal:"9,default,Snapshot" json:"snapshot"`
	Reject       bool        `thrift:"reject,10" frugal:"10,default,bool" json:"reject"`
}

func NewMessage() *Message {
	return &Message{}
}

func (p *Message) InitDefault() {
}

func (p *Message) GetType() (v MessageType) {
	return p.Type
}

func (p *Message) GetSrc() (v int64) {
	return p.Src
}

func (p *Message) GetDst() (v int64) {
	return p.Dst
}

func (p *Message) GetTerm() (v int64) {
	return p.Term
}

func (p *Message) GetLogIndex() (v int64) {
	return p.LogIndex
}

func (p *Message) GetLogTerm() (v int64) {
	return p.LogTerm
}

func (p *Message) GetEntries() (v []*Entry) {
	return p.Entries
}

func (p *Message) GetLeaderCommit() (v int64) {
	return p.LeaderCommit
}

var Message_Snapshot_DEFAULT *Snapshot

func (p *Message) GetSnapshot() (v *Snapshot) {
	if !p.IsSetSnapshot() {
		return Message_Snapshot_DEFAULT
	}
	return p.Snapshot
}

func (p *Message) GetReject() (v bool) {
	return p.Reject
}
func (p *Message) SetType(val MessageType) {
	p.Type = val
}
func (p *Message) SetSrc(val int64) {
	p.Src = val
}
func (p *Message) SetDst(val int64) {
	p.Dst = val
}
func (p *Message) SetTerm(val int64) {
	p.Term = val
}
func (p *Message) SetLogIndex(val int64) {
	p.LogIndex = val
}
func (p *Message) SetLogTerm(val int64) {
	p.LogTerm = val
}
func (p *Message) SetEntries(val []*Entry) {
	p.Entries = val
}
func (p *Message) SetLeaderCommit(val int64) {
	p.LeaderCommit = val
}
func (p *Message) SetSnapshot(val *Snapshot) {
	p.Snapshot = val
}
func (p *Message) SetReject(val bool) {
	p.Reject = val
}

var fieldIDToName_Message = map[int16]string{
	1:  "type",
	2:  "src",
	3:  "dst",
	4:  "term",
	5:  "log_index",
	6:  "log_term",
	7:  "entries",
	8:  "leader_commit",
	9:  "snapshot",
	10: "reject",
}

func (p *Message) IsSetSnapshot() bool {
	return p.Snapshot != nil
}

func (p *Message) Read(iprot thrift.TProtocol) (err error) {

	var fieldTypeId thrift.TType
	var fieldId int16

	if _, err = iprot.ReadStructBegin(); err != nil {
		goto ReadStructBeginError
	}

	for {
		_, fieldTypeId, fieldId, err = iprot.ReadFieldBegin()
		if err != nil {
			goto ReadFieldBeginError
		}
		if fieldTypeId == thrift.STOP {
			break
		}

		switch fieldId {
		case 1:
			if fieldTypeId == thrift.I32 {
				if err = p.ReadField1(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 2:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField2(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 3:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField3(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 4:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField4(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 5:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField5(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 6:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField6(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 7:
			if fieldTypeId == thrift.LIST {
				if err = p.ReadField7(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 8:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField8(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 9:
			if fieldTypeId == thrift.STRUCT {
				if err = p.ReadField9(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 10:
			if fieldTypeId == thrift.BOOL {
				if err = p.ReadField10(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		default:
			if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		}
		if err = iprot.ReadFieldEnd(); err != nil {
			goto ReadFieldEndError
		}
	}
	if err = iprot.ReadStructEnd(); err != nil {
		goto ReadStructEndError
	}

	return nil
ReadStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read struct begin error: ", p), err)
ReadFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d begin error: ", p, fieldId), err)
ReadFieldError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d '%s' error: ", p, fieldId, fieldIDToName_Message[fieldId]), err)
SkipFieldError:
	return thrift.PrependError(fmt.Sprintf("%T field %d skip type %d error: ", p, fieldId, fieldTypeId), err)

ReadFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T read field end error", p), err)
ReadStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T read struct end error: ", p), err)
}

func (p *Message) ReadField1(iprot thrift.TProtocol) error {

	var _field MessageType
	if v, err := iprot.ReadI32(); err != nil {
		return err
	} else {
		_field = MessageType(v)
	}
	p.Type = _field
	return nil
}
func (p *Message) ReadField2(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.Src = _field
	return nil
}
func (p *Message) ReadField3(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.Dst = _field
	return nil
}
func (p *Message) ReadField4(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.Term = _field
	return nil
}
func (p *Message) ReadField5(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.LogIndex = _field
	return nil
}
func (p *Message) ReadField6(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.LogTerm = _field
	return nil
}
func (p *Message) ReadField7(iprot thrift.TProtocol) error {
	_, size, err := iprot.ReadListBegin()
	if err != nil {
		return err
	}
	_field := make([]*Entry, 0, size)
	values := make([]Entry, size)
	for i := 0; i < size; i++ {
		_elem := &values[i]
		_elem.InitDefault()

		if err := _elem.Read(iprot); err != nil {
			return err
		}

		_field = append(_field, _elem)
	}
	if err := iprot.ReadListEnd(); err != nil {
		return err
	}
	p.Entries = _field
	return nil
}
func (p *Message) ReadField8(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.LeaderCommit = _field
	return nil
}
func (p *Message) ReadField9(iprot thrift.TProtocol) error {
	_field := NewSnapshot()
	if err := _field.Read(iprot); err != nil {
		return err
	}
	p.Snapshot = _field
	return nil
}
func (p *Message) ReadField10(iprot thrift.TProtocol) error {

	var _field bool
	if v, err := iprot.ReadBool(); err != nil {
		return err
	} else {
		_field = v
	}
	p.Reject = _field
	return nil
}

func (p *Message) Write(oprot thrift.TProtocol) (err error) {
	var fieldId int16
	if err = oprot.WriteStructBegin("Message"); err != nil {
		goto WriteStructBeginError
	}
	if p != nil {
		if err = p.writeField1(oprot); err != nil {
			fieldId = 1
			goto WriteFieldError
		}
		if err = p.writeField2(oprot); err != nil {
			fieldId = 2
			goto WriteFieldError
		}
		if err = p.writeField3(oprot); err != nil {
			fieldId = 3
			goto WriteFieldError
		}
		if err = p.writeField4(oprot); err != nil {
			fieldId = 4
			goto WriteFieldError
		}
		if err = p.writeField5(oprot); err != nil {
			fieldId = 5
			goto WriteFieldError
		}
		if err = p.writeField6(oprot); err != nil {
			fieldId = 6
			goto WriteFieldError
		}
		if err = p.writeField7(oprot); err != nil {
			fieldId = 7
			goto WriteFieldError
		}
		if err = p.writeField8(oprot); err != nil {
			fieldId = 8
			goto WriteFieldError
		}
		if err = p.writeField9(oprot); err != nil {
			fieldId = 9
			goto WriteFieldError
		}
		if err = p.writeField10(oprot); err != nil {
			fieldId = 10
			goto WriteFieldError
		}
	}
	if err = oprot.WriteFieldStop(); err != nil {
		goto WriteFieldStopError
	}
	if err = oprot.WriteStructEnd(); err != nil {
		goto WriteStructEndError
	}
	return nil
WriteStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write struct begin error: ", p), err)
WriteFieldError:
	return thrift.PrependError(fmt.Sprintf("%T write field %d error: ", p, fieldId), err)
WriteFieldStopError:
	return thrift.PrependError(fmt.Sprintf("%T write field stop error: ", p), err)
WriteStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T write struct end error: ", p), err)
}

func (p *Message) writeField1(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("type", thrift.I32, 1); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI32(int32(p.Type)); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 end error: ", p), err)
}

func (p *Message) writeField2(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("src", thrift.I64, 2); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.Src); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 end error: ", p), err)
}

func (p *Message) writeField3(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("dst", thrift.I64, 3); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.Dst); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 3 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 3 end error: ", p), err)
}

func (p *Message) writeField4(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("term", thrift.I64, 4); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.Term); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 4 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 4 end error: ", p), err)
}

func (p *Message) writeField5(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("log_index", thrift.I64, 5); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.LogIndex); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 5 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 5 end error: ", p), err)
}

func (p *Message) writeField6(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("log_term", thrift.I64, 6); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.LogTerm); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 6 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 6 end error: ", p), err)
}

func (p *Message) writeField7(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("entries", thrift.LIST, 7); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteListBegin(thrift.STRUCT, len(p.Entries)); err != nil {
		return err
	}
	for _, v := range p.Entries {
		if err := v.Write(oprot); err != nil {
			return err
		}
	}
	if err := oprot.WriteListEnd(); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 7 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 7 end error: ", p), err)
}

func (p *Message) writeField8(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("leader_commit", thrift.I64, 8); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.LeaderCommit); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 8 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 8 end error: ", p), err)
}

func (p *Message) writeField9(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("snapshot", thrift.STRUCT, 9); err != nil {
		goto WriteFieldBeginError
	}
	if err := p.Snapshot.Write(oprot); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 9 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 9 end error: ", p), err)
}

func (p *Message) writeField10(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("reject", thrift.BOOL, 10); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteBool(p.Reject); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 10 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 10 end error: ", p), err)
}

func (p *Message) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("Message(%+v)", *p)

}

func (p *Message) DeepEqual(ano *Message) bool {
	if p == ano {
		return true
	} else if p == nil || ano == nil {
		return false
	}
	if !p.Field1DeepEqual(ano.Type) {
		return false
	}
	if !p.Field2DeepEqual(ano.Src) {
		return false
	}
	if !p.Field3DeepEqual(ano.Dst) {
		return false
	}
	if !p.Field4DeepEqual(ano.Term) {
		return false
	}
	if !p.Field5DeepEqual(ano.LogIndex) {
		return false
	}
	if !p.Field6DeepEqual(ano.LogTerm) {
		return false
	}
	if !p.Field7DeepEqual(ano.Entries) {
		return false
	}
	if !p.Field8DeepEqual(ano.LeaderCommit) {
		return false
	}
	if !p.Field9DeepEqual(ano.Snapshot) {
		return false
	}
	if !p.Field10DeepEqual(ano.Reject) {
		return false
	}
	return true
}

func (p *Message) Field1DeepEqual(src MessageType) bool {

	if p.Type != src {
		return false
	}
	return true
}
func (p *Message) Field2DeepEqual(src int64) bool {

	if p.Src != src {
		return false
	}
	return true
}
func (p *Message) Field3DeepEqual(src int64) bool {

	if p.Dst != src {
		return false
	}
	return true
}
func (p *Message) Field4DeepEqual(src int64) bool {

	if p.Term != src {
		return false
	}
	return true
}
func (p *Message) Field5DeepEqual(src int64) bool {

	if p.LogIndex != src {
		return false
	}
	return true
}
func (p *Message) Field6DeepEqual(src int64) bool {

	if p.LogTerm != src {
		return false
	}
	return true
}
func (p *Message) Field7DeepEqual(src []*Entry) bool {

	if len(p.Entries) != len(src) {
		return false
	}
	for i, v := range p.Entries {
		_src := src[i]
		if !v.DeepEqual(_src) {
			return false
		}
	}
	return true
}
func (p *Message) Field8DeepEqual(src int64) bool {

	if p.LeaderCommit != src {
		return false
	}
	return true
}
func (p *Message) Field9DeepEqual(src *Snapshot) bool {

	if !p.Snapshot.DeepEqual(src) {
		return false
	}
	return true
}
func (p *Message) Field10DeepEqual(src bool) bool {

	if p.Reject != src {
		return false
	}
	return true
}

type ConfChangeSingle struct {
	Type    ConfChangeType `thrift:"type,1" frugal:"1,default,ConfChangeType" json:"type"`
	NodeId  int64          `thrift:"node_id,2" frugal:"2,default,i64" json:"node_id"`
	NodeUrl []byte         `thrift:"node_url,3" frugal:"3,default,binary" json:"node_url"`
}

func NewConfChangeSingle() *ConfChangeSingle {
	return &ConfChangeSingle{}
}

func (p *ConfChangeSingle) InitDefault() {
}

func (p *ConfChangeSingle) GetType() (v ConfChangeType) {
	return p.Type
}

func (p *ConfChangeSingle) GetNodeId() (v int64) {
	return p.NodeId
}

func (p *ConfChangeSingle) GetNodeUrl() (v []byte) {
	return p.NodeUrl
}
func (p *ConfChangeSingle) SetType(val ConfChangeType) {
	p.Type = val
}
func (p *ConfChangeSingle) SetNodeId(val int64) {
	p.NodeId = val
}
func (p *ConfChangeSingle) SetNodeUrl(val []byte) {
	p.NodeUrl = val
}

var fieldIDToName_ConfChangeSingle = map[int16]string{
	1: "type",
	2: "node_id",
	3: "node_url",
}

func (p *ConfChangeSingle) Read(iprot thrift.TProtocol) (err error) {

	var fieldTypeId thrift.TType
	var fieldId int16

	if _, err = iprot.ReadStructBegin(); err != nil {
		goto ReadStructBeginError
	}

	for {
		_, fieldTypeId, fieldId, err = iprot.ReadFieldBegin()
		if err != nil {
			goto ReadFieldBeginError
		}
		if fieldTypeId == thrift.STOP {
			break
		}

		switch fieldId {
		case 1:
			if fieldTypeId == thrift.I32 {
				if err = p.ReadField1(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 2:
			if fieldTypeId == thrift.I64 {
				if err = p.ReadField2(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 3:
			if fieldTypeId == thrift.STRING {
				if err = p.ReadField3(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		default:
			if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		}
		if err = iprot.ReadFieldEnd(); err != nil {
			goto ReadFieldEndError
		}
	}
	if err = iprot.ReadStructEnd(); err != nil {
		goto ReadStructEndError
	}

	return nil
ReadStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read struct begin error: ", p), err)
ReadFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d begin error: ", p, fieldId), err)
ReadFieldError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d '%s' error: ", p, fieldId, fieldIDToName_ConfChangeSingle[fieldId]), err)
SkipFieldError:
	return thrift.PrependError(fmt.Sprintf("%T field %d skip type %d error: ", p, fieldId, fieldTypeId), err)

ReadFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T read field end error", p), err)
ReadStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T read struct end error: ", p), err)
}

func (p *ConfChangeSingle) ReadField1(iprot thrift.TProtocol) error {

	var _field ConfChangeType
	if v, err := iprot.ReadI32(); err != nil {
		return err
	} else {
		_field = ConfChangeType(v)
	}
	p.Type = _field
	return nil
}
func (p *ConfChangeSingle) ReadField2(iprot thrift.TProtocol) error {

	var _field int64
	if v, err := iprot.ReadI64(); err != nil {
		return err
	} else {
		_field = v
	}
	p.NodeId = _field
	return nil
}
func (p *ConfChangeSingle) ReadField3(iprot thrift.TProtocol) error {

	var _field []byte
	if v, err := iprot.ReadBinary(); err != nil {
		return err
	} else {
		_field = []byte(v)
	}
	p.NodeUrl = _field
	return nil
}

func (p *ConfChangeSingle) Write(oprot thrift.TProtocol) (err error) {
	var fieldId int16
	if err = oprot.WriteStructBegin("ConfChangeSingle"); err != nil {
		goto WriteStructBeginError
	}
	if p != nil {
		if err = p.writeField1(oprot); err != nil {
			fieldId = 1
			goto WriteFieldError
		}
		if err = p.writeField2(oprot); err != nil {
			fieldId = 2
			goto WriteFieldError
		}
		if err = p.writeField3(oprot); err != nil {
			fieldId = 3
			goto WriteFieldError
		}
	}
	if err = oprot.WriteFieldStop(); err != nil {
		goto WriteFieldStopError
	}
	if err = oprot.WriteStructEnd(); err != nil {
		goto WriteStructEndError
	}
	return nil
WriteStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write struct begin error: ", p), err)
WriteFieldError:
	return thrift.PrependError(fmt.Sprintf("%T write field %d error: ", p, fieldId), err)
WriteFieldStopError:
	return thrift.PrependError(fmt.Sprintf("%T write field stop error: ", p), err)
WriteStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T write struct end error: ", p), err)
}

func (p *ConfChangeSingle) writeField1(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("type", thrift.I32, 1); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI32(int32(p.Type)); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 end error: ", p), err)
}

func (p *ConfChangeSingle) writeField2(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("node_id", thrift.I64, 2); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI64(p.NodeId); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 end error: ", p), err)
}

func (p *ConfChangeSingle) writeField3(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("node_url", thrift.STRING, 3); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteBinary([]byte(p.NodeUrl)); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 3 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 3 end error: ", p), err)
}

func (p *ConfChangeSingle) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("ConfChangeSingle(%+v)", *p)

}

func (p *ConfChangeSingle) DeepEqual(ano *ConfChangeSingle) bool {
	if p == ano {
		return true
	} else if p == nil || ano == nil {
		return false
	}
	if !p.Field1DeepEqual(ano.Type) {
		return false
	}
	if !p.Field2DeepEqual(ano.NodeId) {
		return false
	}
	if !p.Field3DeepEqual(ano.NodeUrl) {
		return false
	}
	return true
}

func (p *ConfChangeSingle) Field1DeepEqual(src ConfChangeType) bool {

	if p.Type != src {
		return false
	}
	return true
}
func (p *ConfChangeSingle) Field2DeepEqual(src int64) bool {

	if p.NodeId != src {
		return false
	}
	return true
}
func (p *ConfChangeSingle) Field3DeepEqual(src []byte) bool {

	if bytes.Compare(p.NodeUrl, src) != 0 {
		return false
	}
	return true
}

type ConfChange struct {
	Transaction ConfChangeTransition `thrift:"transaction,1" frugal:"1,default,ConfChangeTransition" json:"transaction"`
	Changes     []*ConfChangeSingle  `thrift:"changes,2" frugal:"2,default,list<ConfChangeSingle>" json:"changes"`
}

func NewConfChange() *ConfChange {
	return &ConfChange{}
}

func (p *ConfChange) InitDefault() {
}

func (p *ConfChange) GetTransaction() (v ConfChangeTransition) {
	return p.Transaction
}

func (p *ConfChange) GetChanges() (v []*ConfChangeSingle) {
	return p.Changes
}
func (p *ConfChange) SetTransaction(val ConfChangeTransition) {
	p.Transaction = val
}
func (p *ConfChange) SetChanges(val []*ConfChangeSingle) {
	p.Changes = val
}

var fieldIDToName_ConfChange = map[int16]string{
	1: "transaction",
	2: "changes",
}

func (p *ConfChange) Read(iprot thrift.TProtocol) (err error) {

	var fieldTypeId thrift.TType
	var fieldId int16

	if _, err = iprot.ReadStructBegin(); err != nil {
		goto ReadStructBeginError
	}

	for {
		_, fieldTypeId, fieldId, err = iprot.ReadFieldBegin()
		if err != nil {
			goto ReadFieldBeginError
		}
		if fieldTypeId == thrift.STOP {
			break
		}

		switch fieldId {
		case 1:
			if fieldTypeId == thrift.I32 {
				if err = p.ReadField1(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		case 2:
			if fieldTypeId == thrift.LIST {
				if err = p.ReadField2(iprot); err != nil {
					goto ReadFieldError
				}
			} else if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		default:
			if err = iprot.Skip(fieldTypeId); err != nil {
				goto SkipFieldError
			}
		}
		if err = iprot.ReadFieldEnd(); err != nil {
			goto ReadFieldEndError
		}
	}
	if err = iprot.ReadStructEnd(); err != nil {
		goto ReadStructEndError
	}

	return nil
ReadStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read struct begin error: ", p), err)
ReadFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d begin error: ", p, fieldId), err)
ReadFieldError:
	return thrift.PrependError(fmt.Sprintf("%T read field %d '%s' error: ", p, fieldId, fieldIDToName_ConfChange[fieldId]), err)
SkipFieldError:
	return thrift.PrependError(fmt.Sprintf("%T field %d skip type %d error: ", p, fieldId, fieldTypeId), err)

ReadFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T read field end error", p), err)
ReadStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T read struct end error: ", p), err)
}

func (p *ConfChange) ReadField1(iprot thrift.TProtocol) error {

	var _field ConfChangeTransition
	if v, err := iprot.ReadI32(); err != nil {
		return err
	} else {
		_field = ConfChangeTransition(v)
	}
	p.Transaction = _field
	return nil
}
func (p *ConfChange) ReadField2(iprot thrift.TProtocol) error {
	_, size, err := iprot.ReadListBegin()
	if err != nil {
		return err
	}
	_field := make([]*ConfChangeSingle, 0, size)
	values := make([]ConfChangeSingle, size)
	for i := 0; i < size; i++ {
		_elem := &values[i]
		_elem.InitDefault()

		if err := _elem.Read(iprot); err != nil {
			return err
		}

		_field = append(_field, _elem)
	}
	if err := iprot.ReadListEnd(); err != nil {
		return err
	}
	p.Changes = _field
	return nil
}

func (p *ConfChange) Write(oprot thrift.TProtocol) (err error) {
	var fieldId int16
	if err = oprot.WriteStructBegin("ConfChange"); err != nil {
		goto WriteStructBeginError
	}
	if p != nil {
		if err = p.writeField1(oprot); err != nil {
			fieldId = 1
			goto WriteFieldError
		}
		if err = p.writeField2(oprot); err != nil {
			fieldId = 2
			goto WriteFieldError
		}
	}
	if err = oprot.WriteFieldStop(); err != nil {
		goto WriteFieldStopError
	}
	if err = oprot.WriteStructEnd(); err != nil {
		goto WriteStructEndError
	}
	return nil
WriteStructBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write struct begin error: ", p), err)
WriteFieldError:
	return thrift.PrependError(fmt.Sprintf("%T write field %d error: ", p, fieldId), err)
WriteFieldStopError:
	return thrift.PrependError(fmt.Sprintf("%T write field stop error: ", p), err)
WriteStructEndError:
	return thrift.PrependError(fmt.Sprintf("%T write struct end error: ", p), err)
}

func (p *ConfChange) writeField1(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("transaction", thrift.I32, 1); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteI32(int32(p.Transaction)); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 1 end error: ", p), err)
}

func (p *ConfChange) writeField2(oprot thrift.TProtocol) (err error) {
	if err = oprot.WriteFieldBegin("changes", thrift.LIST, 2); err != nil {
		goto WriteFieldBeginError
	}
	if err := oprot.WriteListBegin(thrift.STRUCT, len(p.Changes)); err != nil {
		return err
	}
	for _, v := range p.Changes {
		if err := v.Write(oprot); err != nil {
			return err
		}
	}
	if err := oprot.WriteListEnd(); err != nil {
		return err
	}
	if err = oprot.WriteFieldEnd(); err != nil {
		goto WriteFieldEndError
	}
	return nil
WriteFieldBeginError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 begin error: ", p), err)
WriteFieldEndError:
	return thrift.PrependError(fmt.Sprintf("%T write field 2 end error: ", p), err)
}

func (p *ConfChange) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("ConfChange(%+v)", *p)

}

func (p *ConfChange) DeepEqual(ano *ConfChange) bool {
	if p == ano {
		return true
	} else if p == nil || ano == nil {
		return false
	}
	if !p.Field1DeepEqual(ano.Transaction) {
		return false
	}
	if !p.Field2DeepEqual(ano.Changes) {
		return false
	}
	return true
}

func (p *ConfChange) Field1DeepEqual(src ConfChangeTransition) bool {

	if p.Transaction != src {
		return false
	}
	return true
}
func (p *ConfChange) Field2DeepEqual(src []*ConfChangeSingle) bool {

	if len(p.Changes) != len(src) {
		return false
	}
	for i, v := range p.Changes {
		_src := src[i]
		if !v.DeepEqual(_src) {
			return false
		}
	}
	return true
}
